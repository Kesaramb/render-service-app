...
app.post('/render', async (req, res) => {
  ...
  // Manual test for fabric.Image.fromObject if an image object is present
  const imageObj = Array.isArray(fabricData?.objects)
    ? fabricData.objects.find((obj: any) => obj.type === 'image')
    : null;
  if (imageObj) {
    try {
      (fabric.Image as any).fromObject(imageObj, (img: any, err: any) => {
        if (err) {
          console.error('[Debug] fabric.Image.fromObject error:', err);
        } else {
          console.log('[Debug] fabric.Image.fromObject result:', img);
        }
      });
    } catch (e) {
      console.error('[Debug] Exception calling fabric.Image.fromObject:', e);
    }
  }
  ...
  try {
    ...
    // Log prototype and toBuffer method
    console.log('[Debug] staticCanvas prototype:', Object.getPrototypeOf(staticCanvas));
    console.log('[Debug] staticCanvas.toBuffer:', (staticCanvas as any).toBuffer);

    // Test node-canvas directly
    const testCanvas = createCanvas(100, 100);
    console.log('[Debug] testCanvas.toBuffer:', (testCanvas as any).toBuffer);
